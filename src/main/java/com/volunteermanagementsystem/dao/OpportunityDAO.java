...
public class OpportunityDAO {
    ...
    public boolean create(Opportunity opp) {
        String sql = "INSERT INTO opportunities (org_id, title, description, location, category, slots, event_date) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    public List<Opportunity> findAll() {
        ...
        String sql = "SELECT o.*, org.org_name FROM opportunities o " +
    ...
    public boolean update(Opportunity opp) {
        String sql = "UPDATE opportunities SET title=?, description=?, location=?, category=?, " +
                "slots=?, event_date=?, status=? WHERE opportunity_id=? AND org_id=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public boolean delete(int oppId, int orgId) {
        String sql = "DELETE FROM opportunities WHERE opportunity_id=? AND org_id=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, oppId);
            ps.setInt(2, orgId);
    ...
    private Opportunity mapRow(ResultSet rs) throws SQLException {
        ...
        opp.setOppId(rs.getInt("opportunity_id"));
        ...
        opp.setDeadline(rs.getDate("event_date"));
